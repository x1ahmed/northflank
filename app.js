...
server.on('upgrade', (request, socket, head) => {
    // Check if the upgrade request is for a WebSocket connection
    if (request.headers['sec-websocket-protocol'] === 'vless') { // Or any other identifier for your VLESS protocol
        wss.handleUpgrade(request, socket, head, ws => {
            wss.emit('connection', ws, request);
        });
    } else {
        // If it's not a WebSocket upgrade or not for VLESS, destroy the socket
        socket.destroy();
    }
});
